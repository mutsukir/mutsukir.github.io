...
document.addEventListener('DOMContentLoaded', () => {
  const start = async() => {
    const mindarThree = new MindARThree({
      ...
      imageTargetSrc: 'assets/card_poet.mind',
    ...
    */
    ...
    const geometry_lace = new THREE.PlaneGeometry(3, 3);
    ...
    video_plane.position.set(1.8, -0.1, 0.2);
    ...
    const geometry_button = new THREE.PlaneGeometry(1, 1);
    let selected = 'none';
    ...
    for (let i=0; i<texture_button.length;i++){
      ...
      if (i<texture_button.length/2){
        plane_button[i].position.set(-1.5-1*i, i%(texture_button.length/2), 1.5);
      }
      else{
        plane_button[i].position.set(1.5+(1*(i-2)), i%(texture_button.length/2), 1.5);
      }
    ...
    css_plane.position.set(-1500, 0, 0);
